...
└── deployments/                    # compiled by deploy
    ...
    ├── devrel-assistant-runtime-volume.yaml
    ├── devrel-assistant-runtime.yaml
    ├── devrel-assistant-worker.yaml
...
**Wizard prompts:** agent fields (`name`, `description`, `instructions`, `model`, `skills`, `mcp_servers`), then executor backend (`hermes-runtime` default, with legacy `truefoundry-job` and `truefoundry-service` + Daytona modes available), then optional deployment overrides (`workspace_fqn`, `gateway_url`, `host`, `secrets`, `version`, and Slack allowlists). `--api-only` skips Slack file and Slack optional prompts. Service mode needs `DAYTONA-API-KEY` in the SecretGroup after deploy.
...
<name>-runtime.yaml         → Private stateful Hermes runtime Service
<name>-worker.yaml          → Manual worker Job image for async/maintenance surfaces
...
| `workspace_fqn` | no | `cluster:workspace`; defaults to `TFY_WORKSPACE_FQN` at deploy time |
| `gateway_url` | no | OpenAI-compatible gateway `/v1` URL; defaults to `OPENAI_BASE_URL`, then `https://gateway.truefoundry.ai` |
| `model` | no | Gateway model id; defaults to `openai-main/gpt-5.5` |
| `secrets` | no | SecretGroup name only; defaults to `<name>-hermes-secrets`; `deploy` creates group via API and sets `TFY-API-KEY` + `HERMES-RUN-TOKEN-SECRET` |
...
| `agent_email` | no | AgentMail email address for this assistant; enables `/agentmail/events` |
| `discord` | no | Discord interaction bridge config; allowlists use numeric Discord IDs |
| `skills` | no | Version-pinned `agent-skill:…:N` FQNs; `init` prompts |
| `mcp_servers` | no | MCP Gateway URLs; `init` prompts |
| `executor` | no | `hermes-runtime` default; legacy `truefoundry-job` or `truefoundry-service` |
...
| `AGENTMAIL-API-KEY` | User when `agent_email` is set |
| `AGENTMAIL-WEBHOOK-SECRET` | User when `agent_email` is set |
| `DISCORD-BOT-TOKEN` | User when `discord.enabled` is true |
| `DISCORD-PUBLIC-KEY` | User when `discord.enabled` is true |
| `HERMES-STT-API-KEY` | User/operator for STT gateway override; falls back to `TFY-API-KEY` when unset |
| `HERMES-TTS-API-KEY` | User/operator for TTS gateway override; falls back to `TFY-API-KEY` when unset |
...
| `<name>-runtime-volume.yaml` | Volume | 10Gi RWO PVC at `/workspace/.hermes` on runtime |
...
| `<name>-executor.yaml` | Job or Service | Legacy runner only |

Controller, runtime, worker, and legacy executor manifests embed `tfy-secret://` references to the SecretGroup named in the agent config — they do not contain secret values.
...
- Run dispatched but never completes: inspect `<name>-runtime` logs for the default topology, or `<name>-executor` logs for legacy executor backends.
...
- `tfy apply` succeeds but no rebuild: use `tfy deploy --force -f agents/<name>/deployments/<name>-controller.yaml` and `<name>-runtime.yaml` for default topology.

...
async function request(baseUrl, method, apiPath, body = null) {
...
async function withServer(fn) {
  ...
  const child = spawn(process.execPath, ["control-api/server.mjs"], {
    cwd: repoRoot,
    env: {
      ...process.env,
      PORT: String(port),
...
test("Responses API supports background creation, retrieval, and previous_response_id memory", async () => {
  await withServer(async ({ baseUrl }) => {
    ...
    const work = await request(baseUrl, "GET", `/api/internal/runs/${secondRunId}/work-item`);
...
test("streaming requests return an OpenAI-style unsupported error", async () => {
  await withServer(async ({ baseUrl }) => {
    const result = await request(baseUrl, "POST", "/v1/responses", {
      model: "test-model",
      input: "hello",
      stream: true
    });
    assert.equal(result.response.status, 400);
    assert.equal(result.payload.error.param, "stream");
  });
});

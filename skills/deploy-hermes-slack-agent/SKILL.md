...
7. Deploy (re-runs `validate` first):
   ...
   ```
8. Verify health, backend sessions, snapshotter, Slack URL verification, and one real Slack mention.
...
Snapshot (only when `snapshot` is configured):
- `snapshot.ml_repo` exists and the workspace has write access (not just read).

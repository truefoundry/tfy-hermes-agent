...
**Wizard prompts:** agent fields (`name`, `description`, `instructions`, `model`, `skills`, `mcp_servers`), then optional deployment overrides (`workspace_fqn`, `gateway_url`, `host`, `secrets`, `version`, and Slack allowlists). `--api-only` skips Slack file and Slack optional prompts.
...
Controller, runtime, and worker manifests embed `tfy-secret://` references to the SecretGroup named in the agent config — they do not contain secret values.
...
| LLM (runtime/worker) | Gateway bearer | `TFY-API-KEY` |
...
- Run dispatched but never completes: inspect `<name>-runtime` logs.

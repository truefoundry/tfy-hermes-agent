...
| `HERMES-RUN-TOKEN-SECRET` | Auto-generated by `tfy-hermes-agent init` (saved to `.hermes-secrets.local`). Master secret for per-run HMAC tokens between controller and executor. Not a Slack token. |
